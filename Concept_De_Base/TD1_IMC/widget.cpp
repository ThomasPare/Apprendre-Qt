...
void Widget::afficherInfos()
{
    ...
    ui->textEditAfficheur->append("Bonjour <b>"+prenom+" "+nom+"</b>");
    ...
    ui->textEditAfficheur->append("Votre corpulence est qualifié de "+corpulences[indiceCorpulence]);
}
...
void Widget::devine()
{
    ...
    if (poidsIdeal > 0) {
        ...
        if (ecart >= 0) {
          ui->textEditAfficheur->append("Vous devez perdre "+QString::number(ecart)+" kg\n");
        }
        else {
          ui->textEditAfficheur->append("Vous devez prendre "+QString::number(-ecart)+" kg\n");
        }
    }
}
